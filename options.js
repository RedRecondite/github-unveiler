document.addEventListener('DOMContentLoaded', () => {
  ...
  function loadSettings() {
    ...
    const instantReplaceCheckbox = document.getElementById('instantReplaceCheckbox');
    if (!instantReplaceCheckbox) {
      console.error('Error: instantReplaceCheckbox element not found.');
      return;
    }
    ...
    if (chrome && chrome.storage && chrome.storage.local) {
      chrome.storage.local.get(['githubUnveilerSettings'], result => {
        ...
        instantReplaceCheckbox.checked = settings.instantReplace || false;
      });
    } else {
  ...
  function saveSettings() {
    ...
    if (chrome && chrome.storage && chrome.storage.local) {
      const settings = {
        ...
        instantReplace: instantReplaceCheckbox.checked
      };
  ...
  const instantReplaceCheckbox = document.getElementById('instantReplaceCheckbox');
  if (instantReplaceCheckbox) {
    instantReplaceCheckbox.addEventListener('change', saveSettings);
  }
